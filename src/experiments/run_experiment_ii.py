...
from src.preprocessing.transform import preprocesar_para_modelo_ii, cargar_datos
...
print("\n--- Experimento: MODELO II (sin T1 y T2) ---")
df = cargar_datos("data/rendimiento_estudiantes_train.csv")
X, y = preprocesar_para_modelo_ii(df)
...
plot_feature_importances(importancia_lr, "Linear_Regression_II")
...
plot_feature_importances(importancia_lr_f, "Linear_Regression_II_FILTRADO")
pd.Series(cols_lr).to_csv("resultados/variables_lr_ii_filtrado.csv", index=False)
...
plot_feature_importances(importancia_rf, "Random_Forest_II")
...
plot_feature_importances(importancia_rf_f, "Random_Forest_II_FILTRADO")
pd.Series(cols_rf).to_csv("resultados/variables_rf_ii_filtrado.csv", index=False)
...
plot_feature_importances(importancia_mlp, "MLP_II")
...
if len(X_train_mlp.columns) == 0:
    ...
    plot_feature_importances(importancia_mlp_f, "MLP_II_FILTRADO")
    importancia_mlp_f.to_csv("resultados/importancia_mlp_ii_filtrado.csv")
    pd.Series(cols_mlp).to_csv("resultados/variables_mlp_ii_filtrado.csv", index=False)


# 5. Visualización de métricas
plot_metric_bars(resultados, "Experimento_II")
...
pd.DataFrame(metricas).to_csv("resultados/metricas_experimento_II.csv", index=False)
print("Métricas guardadas en resultados/metricas_experimento_II.csv")

importancia_lr.to_csv("resultados/importancia_lr_ii.csv")
importancia_lr_f.to_csv("resultados/importancia_lr_ii_filtrado.csv")
pd.Series(importancia_rf).to_csv("resultados/importancia_rf_ii.csv")
pd.Series(importancia_rf_f).to_csv("resultados/importancia_rf_ii_filtrado.csv")
importancia_mlp.to_csv("resultados/importancia_mlp_ii.csv")
